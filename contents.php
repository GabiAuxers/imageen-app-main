   ...
   // $logger->log();
    ...
    if (!empty($m)) {
        ...
        if ($result->num_rows > 0) {
            //we use a different function here for fetching
            // before -> $row = mysqli_fetch_array($result);
            $row = $result->fetch_assoc();
            // Isset changes - Modified - 06/03/2023
            $a = isset($row["ACCESO"]) ? $row["ACCESO"] : 0;
        } else {
            $a = 0;
        }
    }
...
<<!--New PHP structure -  23/03/2023 - 12:45-->
<!--.htmlentities inclued, for XSS attacks and security problems. -  28/03/2023 -->
<?php
if ($x !== "" || $visualizacion_usuario === 1) {
    if ($x !== "") {
        echo '<script src="' . htmlentities($ruta_admin) . '/ficheros/puntos' . htmlentities($x) . '.xml"></script>';
    } else {
        echo '<script src="' . htmlentities($ruta_admin) . '/ficheros/puntosImageen' . htmlentities($l) . '.xml"></script>';
    }
    if ($t === 3) {
?>

<body
    onload="onLoadContents(3, false, '<?= htmlentities($p) ?>', '<?= htmlentities($m) ?>', '<?= htmlentities($u) ?>', '<?= htmlentities($a) ?>');">
    <?php
    } else {
        ?>

    <body
        onload="onLoadContents(1, false, '<?= htmlentities($p) ?>', '<?= htmlentities($m) ?>', '<?= htmlentities($u) ?>', '<?= htmlentities($a) ?>');">
        <?php
        }
    } else {
            ?>
        ...
        <body
            onload="onLoadContents(2, false, '<?= htmlentities($p) ?>', '<?= htmlentities($m) ?>', '<?= htmlentities($u) ?>', '<?= htmlentities($a) ?>');">
            <?php
        }
            ?>
            <!--Ripley.js javascript library for styling map-->
            <div id="main">
                <?php
                ...
                <div class="container-fluid p-0">
                    <div class="bg-dark text-white text-center"><a href="contents.php"
                            style="text-decoration:none;color:white;"><span class="me-2 fs-16px">Cerrar para volver a
                                Imageen</span><i class="ri-close-fill fs-22px me-3"></i></a></div>
                    <div id="map"></div>
                </div>
                <?php
                    } else {
                ...
                <!--Modified 23/03/2023 - 13:38 | use of fetch_array() function instead of mysqli_fetch_array()
                    to get the associated array of query results,
                    because the mysqli_fetch_array() function is obsolet in PHP 7.3.33.
                    Also the call to the mysqli_data_seek() function was deleted because was useless in this case.-->
                <?php
                    $conn = new mysqli($db_server, $db_username, $db_userpassword, $db_name);
                    mysqli_set_charset($conn, "utf8");
                    $sql =
                        "SELECT CODIGO, NOMBRE FROM CIUDADES WHERE ESTADO = 1 ORDER BY ORDEN ";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        $num_ciudad = 0;
                        //variable inicializated
                        $nombreciudadciclo = "";
                ...
                <div class="mt-5 text-center">
                    <h3><b><?php echo isset($nombreciudadciclo) ? $nombreciudadciclo : ""; ?></b></h3>
                    ...
                    <div class="swiper swiper-ciudades pb-4">
                        <div class="swiper-wrapper">
                            ...
                            <div class="swiper-slide">
                                ...
                                <div
                                    class="titulo-POIs fs-4 position-absolute top-0 mt-3 px-1 text-start bg-black-transparent-2">
                                    <?= $titulopunto ?></div>
                                <div class="position-absolute bottom-0" style="width:100%;">
                                    <i style="color: rgba(255,255,255,0.75);" class="ri-play-mini-line ri-3x"></i>
                                </div>
                                <?php if ($clientepunto == "") { ?>
                                <a class="position-absolute w-100 h-100" role="button" href="#contentx"
                                    onclick="loadContents({'codigo': '<?php echo $codigopunto; ?>', 'nombre': '<?php echo $titulopunto; ?>'})"></a>
                                <?php } else { ?>
                                <a class="position-absolute w-100 h-100" role="button" href="#pointx"
                                    data-bs-toggle="modal"
                                    onclick="loadPoint({'codigo': '<?php echo $codigopunto; ?>', 'imagen': '<?php echo $imagenpunto; ?>', 'cliente': '<?php echo $clientepunto; ?>', 'nombre': '<?php echo $titulopunto; ?>', 'descripcion': '<?php echo $descripcionpunto; ?>', 'icono': '<?php echo $iconog; ?>'})"></a>
                                <?php } ?>
                            </div>
                            <?php } ?>
                        </div>

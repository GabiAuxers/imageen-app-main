...
var rutaAdmin = "https://admin.imageen.net";
...
function showWithPosition(position) {
	...
	for (var i = 0; i < data.points.length; i++) {
		...
		if (tipo == '0') { //POI Imageen
			var marker = new google.maps.Marker({ map: map, title: nombreb, label: { text: nombreb, color: "#2765A0", fontFamily: "'Raleway', sans-serif", fontWeight: "bold" }, position: latLng, icon: { url: rutaAdmin + "/data/imagenes/" + icono, labelOrigin: { x: 18, y: 40 } }, animation: "DROP" });
			marker.parametros = { "codigo": codigo, "imagen": imagen, "cliente": cliente, "nombre": nombreb, "distancia": distance, "materiales": contenidos, "descripcion": descripcionb, "icono": iconog, "tipo": "imageen" };

		} else { // POI cliente
			var marker = new google.maps.Marker({ map: map, title: nombreb, position: latLng, icon: { url: rutaAdmin + "/data/imagenes/" + icono, labelOrigin: { x: 18, y: 40 } } });
			marker.parametros = { "codigo": codigo, "imagen": imagen, "cliente": cliente, "nombre": nombreb, "distancia": distance, "materiales": contenidos, "descripcion": direccionb, "icono": iconog, "tipo": "cliente", "localizacion": link };
		}
...
function showWithoutPosition() {
	...
	for (var i = 0; i < data.points.length; i++) {
		...
		if (tipo == '0') {
			var marker = new google.maps.Marker({ map: map, title: nombreb, label: { text: nombreb, color: "#2765A0", fontFamily: "'Raleway', sans-serif", fontWeight: "bold" }, position: latLng, icon: { url: rutaAdmin + "/data/imagenes/" + icono, labelOrigin: { x: 18, y: 40 } }, animation: "DROP" });
			marker.parametros = { "codigo": codigo, "imagen": imagen, "cliente": cliente, "nombre": nombreb, "distancia": null, "materiales": contenidos, "descripcion": descripcionb, "icono": iconog };
		} else {
			var marker = new google.maps.Marker({ map: map, title: nombreb, position: latLng, icon: { url: rutaAdmin + "/data/imagenes/" + icono, labelOrigin: { x: 18, y: 40 } } });
			...
			marker.parametros = { "codigo": codigo, "imagen": imagen, "cliente": cliente, "nombre": nombreb, "distancia": null, "materiales": contenidos, "descripcion": direccionb, "icono": iconog, "tipo": "cliente", "localizacion": link};
		}
...
function sizemap() {
	var x = screen.height;
	document.getElementById('map').style.height = x + 'px';
}
...
function cambiarVisual(v, l) {
	document.getElementById("enlace-cambiar-visual").classList.add("d-none");
	document.getElementById("spinner-cambiar-visual").classList.remove("d-none");
	var dataString = "v=" + v;
	$.ajax({
		type: "POST",

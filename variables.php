...
if ($auth == 1) {
    ...
    if ($m != "") {
        ...
        $stmt = $conn->prepare("SELECT ACCESO FROM MATERIALES WHERE CODIGO=?");
        $stmt->bind_param("s", $m2);
        $stmt->execute();
    ...
    if ($x != "") {
        ...
        mysqli_set_charset($conn, 'utf8');
         "SELECT NOMBRE, IMAGEN, TEXTO1, INICIO FROM CLIENTES WHERE CODIGO ='$x'";
        $stmt2 = $conn->prepare("SELECT NOMBRE, IMAGEN, TEXTO1, INICIO FROM CLIENTES WHERE CODIGO =?");
